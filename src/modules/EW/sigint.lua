...
---@param unitData SBJ__Battery unit data (only Battery units are processed for movement)
...
local function isUnitEmitting(config, unit, unitData, enemySide)
  ...
  if not unitData.position.RL.area or #unitData.position.RL.area == 0 then
    return false, "No RL area defined"
  end

  local lastCoursePoint = unit.course[courseCount]
  local isLeavingRL = not isInArea(enemySide, lastCoursePoint, unitData.position.RL.area)
...
---@param unitData SBJ__Battery|SBJ__C2 unit data
...
---@param saveData SBJ__SaveData save data
---@param side string side name
---@param units SBJ__Battery[]|SBJ__C2[] unit list (mobile missile launchers)
---@param isShown boolean whether to show notification
