...
MoveToStagingAreaArea = require('modules.landingOps.moveToStagingArea').MoveToStagingArea
GetUnitsInAnchorageArea = require('src.modules.landingOps.prepareAmphibiousAssault').GetUnitsInAnchorageArea
CreateCargoMissions = require('src.modules.landingOps.prepareAmphibiousAssault').CreateCargoMissions
TransferAndAssign = require('src.modules.landingOps.prepareAmphibiousAssault').TransferAndAssign
SetCoursesForLSTs = require('modules.landingOps.launchAmphibiousAssault').SetCoursesForLSTs
SetLandingMissionStartTime = require('modules.landingOps.launchAmphibiousAssault').SetLandingMissionStartTime
CountContactsInArea = require('modules.landingOps.launchAmphibiousAssault').CountContactsInArea
...
-- local function moveToStagingArea(saveData)
...
if saveData.c.PHIBOP.isShipsStartedMoving and IsAfterStartTime(saveData.c.PHIBOP.startTime) then
  local hasIssuedShipMovementOrder = MoveToStagingArea(saveData, CONFIG, units)
...
if saveData.c.PHIBOP.isWaitingForShipArrival then
  local result = GetUnitsInAnchorageArea(CONFIG, units)
  local hasArrived = GetCount(result.units) > 15 and not result.isUnitMoving

  if hasArrived then
    local creatingCompleted = CreateCargoMissions(CONFIG)
    local transferingCompleted = TransferAndAssign(CONFIG, result.units)
...
if saveData.c.PHIBOP.isWaitingForAmphibiousAssault then
  ...
  local contactCount = CountContactsInArea(contacts, initialLocations[1].airLandingZone)
  ...
  if shouldLaunchAmphibiousAssault then
    local settingStartTimeCompleted = SetLandingMissionStartTime(CONFIG, saveData)
    local settingCoursesCompleted = SetCoursesForLSTs(CONFIG, units)
