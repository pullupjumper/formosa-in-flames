...
function SetupMenuPage() {
  ...
  const [airbases, setAirbases] = useState(() =>
    structuredClone(injectedData.airbases),
  );
  const jammers = injectedData.jammers;
  const missileSystems = injectedData.missileSystems;
  const [deployedJammers, setDeployedJammers] = useState<
  ...
  // Budget handlers
  const spendBudget = useCallback(
    (amount: number = CONFIG.LOADOUT_COST): boolean => {
  ...
  const refundBudget = useCallback(
    (amount: number = CONFIG.LOADOUT_COST): void => {
      setBudget((prev) => ({
        ...prev,
        current: prev.current + amount,
        used: prev.used - amount,
  ...
  const updateAirbase = useCallback(
    (index: number, updater: (base: Airbase) => Airbase) => {
    ...
    <div className="flex h-screen flex-col">
      ...
      <div className="flex h-12.5 items-center justify-between border-b border-dark-border bg-dark-panel px-5">
        <div className="text-sm font-medium text-text-secondary">
          <span className="text-text-primary">TAIWAN</span> / DEPLOYMENT
          PLANNING
        </div>
      </div>
          ...
          <JammersTab
            ...
            onDeployJammer={(index, lat, lng) => {
              setDeployedJammers((prev) =>
                new Map(prev).set(index, { lat, lng }),
              );
            }}
