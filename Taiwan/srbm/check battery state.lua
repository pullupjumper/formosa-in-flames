...
for _, battery in ipairs(CONFIG.t.srbm.batteries) do
    ...
    if group then
        if battery.reloadStartTime == nil then
            battery.reloadStartTime = ScenEdit_CurrentTime() - CONFIG.t.srbm.const.reloadTime
        end

        local diff = ScenEdit_CurrentTime() - battery.reloadStartTime
        local isMoreThanReloadTime = (battery.reloadStartTime ~= nil and diff >= CONFIG.t.srbm.const.reloadTime)
